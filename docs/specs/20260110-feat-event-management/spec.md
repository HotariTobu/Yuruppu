...
- [ ] NFR-002: 作成者非公開のイベントでは、一覧表示や詳細表示で作成者情報が「匿名」と表示される（イベント作成者自身が取得する場合を除く）
...
- **Then**:
  - 全イベントが返される
  - 開始日時の昇順でソートされる
  - 作成者非公開のイベントは、作成者以外のユーザーには「匿名」と表示される
...
### AC-017: Anonymous creator visible to self [NFR-002]

- **Given**: ユーザーが作成者非公開でイベントを作成した
- **When**: 作成者自身がそのイベントの詳細を取得する
- **Then**:
  - 作成者情報（自分自身）が表示される
